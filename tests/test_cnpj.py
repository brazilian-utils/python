...
if __name__ == "__main__":
    main()
class TestAlphanumericCNPJ(TestCase):
    def test_is_valid_alphanumeric(self):
        # Alphanumeric CNPJs generated by generate() must be valid
        for _ in range(100):
            cnpj = generate(alphanumeric=True)
            self.assertIs(is_valid(cnpj), True)

    def test_is_valid_rejects_invalid_alphanumeric(self):
        # Wrong checksum
        self.assertIs(is_valid("AAAAAAAA000100"), False)
        # All same chars
        self.assertIs(is_valid("AAAAAAAAAAAAAA"), False)
        # Invalid character
        self.assertIs(is_valid("AAAAAAA!000100"), False)

    def test_validate_alphanumeric(self):
        for _ in range(100):
            cnpj = generate(alphanumeric=True)
            self.assertIs(validate(cnpj), True)

    def test_char_to_val(self):
        from brutils.cnpj import _char_to_val
        self.assertEqual(_char_to_val("0"), 0)
        self.assertEqual(_char_to_val("9"), 9)
        self.assertEqual(_char_to_val("A"), 10)
        self.assertEqual(_char_to_val("Z"), 35)

    def test_is_valid_chars(self):
        from brutils.cnpj import _is_valid_chars
        self.assertTrue(_is_valid_chars("ABC123"))
        self.assertFalse(_is_valid_chars("ABC!23"))
        self.assertFalse(_is_valid_chars("abc123"))
    ...
    def test_generate_alphanumeric_has_letter(self):
        # Alphanumeric CNPJ must contain at least one letter in the base
        for _ in range(100):
            cnpj = generate(alphanumeric=True)
            self.assertTrue(any(c.isalpha() for c in cnpj[:12]))

    def test_generate_numeric_backward_compat(self):
        # generate() without alphanumeric=True must still return numeric CNPJ
        for _ in range(100):
            cnpj = generate()
